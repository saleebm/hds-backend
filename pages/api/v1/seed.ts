import {
  ...
  EmployeeRoleCapability,
} from '@prisma/client'
// product sample
import applianceData from '../../../data/json/appliances.json'
// customers from tab delimited csv
import customerData from '../../../data/json/customers.json'
// random words for emp addresses...
import randomWords from '../../../data/json/dictionary.json'
// employees
import employeeData from '../../../data/json/employees.json'
// locations
import locationData from '../../../data/json/location.json'
...
/**
 * Must add seed_secret to params to init
 */
export default handler(async (req) => {
  const secret =
    req.query && 'seed_secret' in req.query && req.query['seed_secret']
  ...
  async function createProducts(locations: StoreLocations[]) {
  ...
  async function main() {
    ...
    for (const emp of employeeData) {
      ...
      employeesCreateInput.push(
        prisma.employee.create({
          data: {
            positionName: JOB as EmployeePositionName,
            /** TODO: only the president gets to do any work? What kind of company is this? */
            roleCapability:
              JOB === 'PRESIDENT_CEO'
                ? ('READ_WRITE' as EmployeeRoleCapability)
                : ('NONE' as EmployeeRoleCapability),
